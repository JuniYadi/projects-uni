...
| Skill | When to use |
|-------|-------------|
| `expo:expo-ui` | **@expo/ui** — building native UI with Host, Button, Switch, Picker, FieldGroup, List, etc. Use BEFORE writing any `@expo/ui` code. |
| `expo:building-native-ui` | Building beautiful apps with Expo Router — styling, navigation, animations, native tabs. |
| `expo:native-data-fetching` | Implementing/debugging ANY network request or data fetching. |
| `expo:expo-tailwind-setup` | Tailwind CSS v4 setup with NativeWind v5. |
| `expo:expo-api-routes` | Creating API routes in Expo Router with EAS Hosting. |
| `expo:expo-module` | Creating native modules with Expo Modules API. |
| `expo:expo-dev-client` | Building/distributing dev clients. |
| `expo:expo-deployment` | Build & submit to App Store / Play Store. |
| `expo:expo-cicd-workflows` | Writing EAS workflow YAML files. |
| `expo:upgrading-expo` | Upgrading Expo SDK versions. |
| `expo:expo-examples` | Official `with-*` example projects. |
| `expo:use-dom` | Using Expo DOM components. |
| `expo:expo-observe` | EAS Observe app metrics. |
...
|-------------|-------------|
| `context7` | Fetch current docs for any library/framework — use BEFORE writing code against unfamiliar APIs. |
| `expo` (plugin) | Manage EAS builds, workflows, app store reviews, TestFlight, crash data. |
